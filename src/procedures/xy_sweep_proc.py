...
Description: 
    Define the full experiment procedure for a sweep along both the X and Y axes (grid).
...
class XY_Sweep(Procedure):
    ...
    def startup(self):
        ...
        log.info(f"Move stage to ({self.x_min}mm, {self.y_min}mm)")
    ...
    def execute(self):
        ...
        for i in range(len(self.x_values)):
            ...
            for item in [self.b, -self.b] * int(self.repeat_num):
                ...
                # Inner loop over y positions
                for y in range(len(self.y_values)):

                    # We move the stage to the next position along y
                    stage.move_y_to(self.y_values[y])
